...
public class Euler_J {
	...
	public BigInteger Problem15() {
		...
		return Utility_J.combinacoes(40,20);
	}
	...
	public int Problem23() {
		for (int i=1;i<isAbundante.length;i++) {
			isAbundante[i] = Utility_J.isAbundante(i);
		}
		int sum=0;
		for (int i=1;i<=LIMITE23;i++) {
			if (!Utility_J.isSoma2Abundantes(i)) {
				sum +=i;
			}
		}
		return sum;
	...
	public String Problem24() {
		...
		for (int i = 0; i < 999999; i++) {
			if (!Utility_J.nextPermutation(array))
	...
	public int Problem30() {
		...
		for (int i=2;i<1000000;i++) {
			if (i==Utility_J.somaElevadoCinco(i)) {
				sum+=i;
			}
		}
		return sum;
	...
	public int Problem53() {
		...
		for (int i=1;i<=100;i++) {
			for (int j=0;j<=i;j++) {
				if (Utility_J.binomial(i, j).compareTo(Milhao)>0) {
					count++;
				}
			}
		}
